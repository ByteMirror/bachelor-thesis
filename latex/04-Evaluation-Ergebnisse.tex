...
Tabelle~\autoref{tab:benchmark-results} zeigt die aggregierten Ergebnisse der Performance-Messungen über alle drei Szenarien.
...
Die Analyse des Speicherverlaufs über die 500 Iterationen des Stress-Tests offenbart ein systematisches Speicherleck. Abbildung~\autoref{fig:memory-leak} zeigt den linearen Anstieg des Heap-Verbrauchs.
...
Um die Ursache des Memory Leaks zu lokalisieren, wurde der Benchmark-Iterationszyklus analysiert. Abbildung~\autoref{fig:benchmark-flow} zeigt den Ablauf einer einzelnen Iteration mit den beteiligten Komponenten.
...
Der in Abbildung~\autoref{fig:benchmark-flow} dargestellte Iterationszyklus durchläuft folgende Schritte:
...
Die Benchmark-Ergebnisse zeigen ein positives Skalierungsverhalten der Rendering-Performance (vgl. Abbildung~\autoref{fig:scaling-analysis}).
