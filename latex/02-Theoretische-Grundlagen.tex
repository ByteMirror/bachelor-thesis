...
\begin{figure}[t!]
    ...
    \caption{Token-Interaktion in Atlas VTT mit Ressourcenbalken, Statuseffekten und Kontextmenü.}
...
Abbildung~\ref{fig:token-demo} zeigt die Token-Implementierung in Atlas VTT mit konfigurierbaren Ressourcenbalken, Statuseffekt-Tags und einem Kontextmenü für häufige Operationen.

\clearpage
\begin{figure}[t!]
    \centering
    \includegraphics[width=0.95\textwidth]{bilder/statblock-showcase.png}
    \caption{Statblock-Integration in Atlas VTT: Markdown-Quelldatei (links) und gerenderter Statblock (rechts).}
...
Die Struktur eines Statblocks folgt dabei dem jeweiligen Regelsystem. In Dungeons \& Dragons 5e beispielsweise umfasst ein Statblock: Attribute (Stärke, Geschicklichkeit, Konstitution, Intelligenz, Weisheit, Charisma), abgeleitete Werte (Rüstungsklasse, Initiative, Geschwindigkeit), Fertigkeiten und Rettungswürfe, spezielle Fähigkeiten und verfügbare Aktionen. Moderne \ac{VTT}s ermöglichen es, diese Werte direkt anzuklicken – ein Klick auf eine Angriffsfähigkeit würfelt automatisch den Angriff und berechnet den Schaden unter Berücksichtigung aller Modifikatoren.

Abbildung~\ref{fig:statblock-showcase} demonstriert die Statblock-Implementierung in Atlas VTT, die Obsidians Markdown-Fähigkeiten nutzt: Die editierbare Quelldatei (links) wird automatisch als interaktiver Statblock mit klickbaren Würfelformeln gerendert (rechts).

\clearpage
\begin{figure}[t!]
    \centering
    \includegraphics[width=0.9\textwidth]{bilder/fog-of-war-demo.jpeg}
    \caption{Fog of War in Atlas VTT: Unerforschte Bereiche werden für Spieler verborgen.}
...
Abbildung~\ref{fig:fog-of-war-demo} illustriert die Fog-of-War-Implementierung in Atlas VTT, bei der unerforschtes Terrain (schwarz) für Spieler vollständig verborgen bleibt.
