...
\begin{itemize}
    ...
    \item Die Frame Time von durchschnittlich 8,33 \ac{ms} liegt unter dem Schwellenwert für flüssige Interaktion (16,67 \ac{ms} für 60 \ac{FPS}\autocite{Nielsen1993ResponseTimes})
...
Die Implementierung von Atlas \ac{VTT} demonstriert, dass performante \ac{VTT}-Plugins in Obsidian grundsätzlich realisierbar sind. Die gemessenen 120 \ac{FPS} und Frame Times von 8,33 \ac{ms} übertreffen die in \autoref{sec:Anforderungsanalyse} definierten Anforderungen (>30 \ac{FPS}, <100 \ac{ms} Eingabelatenz). Die Electron-Umgebung stellt somit keine prinzipielle Barriere für Echtzeit-Rendering-Anwendungen dar.

\paragraph{Erfolgsfaktoren}
Drei zentrale Faktoren ermöglichen die erreichte Performance:

\begin{enumerate}
    \item \textbf{Moderne Rendering-Engine}: PIXI.js v8 mit \ac{WebGL}-Backend nutzt \ac{GPU}-Beschleunigung effektiv aus. Die automatischen Optimierungen wie Sprite-Batching und Culling\autocite{PixiPerf2024} sowie die TilingSprite-basierte Grid-Implementierung (vgl. \autoref{subsec:grid-system}) reduzieren den \ac{CPU}-Overhead und die Anzahl der Draw-Calls.

    \item \textbf{Zustandsmanagement mit Zustand}: Die reaktive Store-Architektur mit \texttt{subscribeWithSelector} ermöglicht gezielte \ac{UI}-Updates ohne vollständiges Re-Rendering. Die Trennung von Rendering-State und Persistenz-State vermeidet unnötige Disk-\ac{I/O} während der Laufzeit.
...
Neben den erreichten Performance-Werten zeigt die Evaluation auch Grenzen auf:

\begin{itemize}
    \item Das identifizierte Memory Leak bei intensiver Token-Manipulation deutet auf Komplexität im Lifecycle-Management von PIXI.js-Objekten hin. Die korrekte Freigabe von Graphics-Objekten, Event-Listenern und Texture-Referenzen erfordert präzise Implementierung.

    \item Die Bundle-Größe des Plugins (ca. 2,8 \ac{MB}) ist primär durch PIXI.js und React-Abhängigkeiten bedingt. Die Single-Bundle-Architektur von Obsidian verhindert eine Aufteilung in on-demand geladene Module.

    \item Der Speicherverbrauch von ca. 10 \ac{MB} pro Token kann bei sehr großen Szenarien (>100 Token) relevant werden.
\end{itemize}

\paragraph{Fazit zur Forschungsfrage}
Die Forschungsfrage kann wie folgt beantwortet werden: \ac{VTT}-Plugins können in Obsidian performant implementiert werden, wenn geeignete Technologien (\ac{WebGL}-basiertes Rendering, reaktives State Management) und Best Practices (asynchrones Loading, Texture-Caching, inkrementelle Updates) angewendet werden. Die technischen Limitierungen von Electron sind für die getesteten Szenarien mit bis zu 100 Token nicht performance-kritisch. Die Herausforderungen liegen weniger in der Rendering-Performance als vielmehr im korrekten Memory Management und der Bundle-Größen-Optimierung.

\section{Limitationen und kritische Reflexion}
\label{sec:Limitationen}

Die Aussagekraft der Ergebnisse wird durch methodische und technische Einschränkungen begrenzt. \autoref{subsec:MethodischeEinschraenkungen} diskutiert die Grenzen der gewählten Evaluationsmethodik. \autoref{subsec:TechnischeLimitationen} beschreibt technische Constraints der Obsidian-Plattform. \autoref{subsec:KritischeWuerdigung} reflektiert Stärken und Schwächen der Arbeit.

\subsection{Methodische Einschränkungen}
\label{subsec:MethodischeEinschraenkungen}
...
Alle Messungen wurden ausschließlich auf einem Apple Silicon Mac (M1 Pro) unter macOS durchgeführt. Die Performance-Charakteristika können auf anderen Plattformen (Windows, Linux) und Prozessorarchitekturen (Intel, AMD) abweichen. Insbesondere die \ac{GPU}-Integration von PIXI.js verhält sich möglicherweise unterschiedlich auf verschiedenen Grafiktreibern. Eine Cross-Platform-Evaluation wäre für besser generalisierbare Aussagen erforderlich.
...
\label{subsec:TechnischeLimitationen}
...
Das Plugin ist an die in Obsidian gebündelte Electron-Version (32.2.5 zum Testzeitpunkt) gebunden. Neuere Electron-Versionen mit verbesserten V8- und Chromium-Engines stehen erst nach entsprechenden Obsidian-Updates zur Verfügung\autocite{ElectronPerformanceGuide2024}. Dies limitiert die Möglichkeit, von Performance-Verbesserungen in neueren Electron-Releases zu profitieren.

\paragraph{PIXI.js-Versionskonflikt}
Die Notwendigkeit, PIXI.js v8 separat zu bundlen (statt die globale v7-Version von Obsidian zu nutzen), erhöht die Bundle-Größe um ca. 500 \ac{KB}. Dieser Trade-off zugunsten höherer Performance und erweiterter \ac{API}-Features führt zu erhöhtem Ressourcenverbrauch.
...
\begin{enumerate}
    \item \textbf{Frühzeitige Performance-Tests}: Performance-Tests sollten von Beginn an in den Entwicklungsprozess integriert werden, nicht erst als finale Evaluation
...
Dieser Abschnitt skizziert mögliche Weiterentwicklungen und offene Forschungsfragen. \autoref{subsec:WeiterfuehrendeForschung} identifiziert potenzielle Optimierungsstrategien und offene Forschungsfragen. \autoref{subsec:PraktischeAnwendung} diskutiert die praktische Anwendbarkeit der Ergebnisse. \autoref{subsec:TechnologischeTrends} ordnet die Arbeit in aktuelle technologische Entwicklungen ein.

\subsection{Weiterführende Forschung}
\label{subsec:WeiterfuehrendeForschung}
...
\begin{itemize}
    \item \textbf{Code-Splitting und Lazy Loading}: Eine Erweiterung der Obsidian-Plugin-\ac{API} zur Unterstützung von dynamischen Imports könnte erhebliche Startup-Zeit-Verbesserungen ermöglichen, indem Features erst bei Bedarf geladen werden.

    \item \textbf{Web Workers für \ac{CPU}-intensive Operationen}: Rechenintensive Operationen wie Hexagonal-Grid-Berechnungen, Asset-Indexierung oder Fog-of-War-Algorithmen könnten in Web Workers ausgelagert werden, um Main-Thread-Blocking zu reduzieren\autocite{ElectronPerformanceGuide2024}.
...
\begin{itemize}
    \item \textbf{WebGPU}: Die aufkommende WebGPU-\ac{API} könnte \ac{WebGL} ablösen und direkteren Zugriff auf \ac{GPU}-Ressourcen ermöglichen. Eine Migration von PIXI.js \ac{WebGL} zu PIXI.js WebGPU könnte Performance-Verbesserungen bringen.
...
\begin{itemize}
    ...
    \item Welche Performance-Trade-offs ergeben sich bei der Integration weiterer \ac{VTT}-Features (z.B. Multiplayer-Synchronisation, erweiterte Fog-of-War-Algorithmen)?
...
\label{subsec:PraktischeAnwendung}

Die entwickelte Lösung und die gewonnenen Erkenntnisse haben Implikationen über das konkrete Projekt hinaus.

\paragraph{Integration in bestehende \ac{VTT}-Ökosysteme}

Atlas \ac{VTT} positioniert sich als Ergänzung zu etablierten \ac{VTT}-Plattformen wie FoundryVTT oder Roll20, nicht als deren Ersatz. Die Stärke liegt in der Integration mit dem Obsidian-Ökosystem: Kampagnennotizen, Charakterbögen und Session-Logs können direkt neben der virtuellen Spielfläche verwaltet werden. Eine potenzielle Weiterentwicklung könnte bidirektionale Synchronisation mit externen \ac{VTT}-Systemen umfassen, etwa durch Import/Export von Foundry-Szenen oder Integration mit D\&D Beyond-Charakterdaten.
...
Als Open-Source-Projekt bietet Atlas \ac{VTT} Möglichkeiten für Community-Beiträge. Die modulare Service-Architektur erleichtert das Hinzufügen neuer Features ohne Risiko für bestehende Funktionalität. Potenzielle Erweiterungsbereiche umfassen:
...
\label{subsec:TechnologischeTrends}
...
\begin{itemize}
    \item \textbf{WebGPU}: Der Nachfolger von \ac{WebGL} bietet direkteren Zugriff auf \ac{GPU}-Ressourcen und könnte signifikante Performance-Verbesserungen für Rendering-intensive Anwendungen ermöglichen. PIXI.js arbeitet bereits an WebGPU-Unterstützung.
    ...
    \item \textbf{\ac{CSS} Container Queries und Subgrid}: Vereinfachen responsive Layouts ohne JavaScript und könnten \ac{UI}-Performance verbessern.

    \item \textbf{View Transitions \ac{API}}: Ermöglicht flüssige Animationen zwischen View-States mit minimaler Implementierungskomplexität.
\end{itemize}
...
\begin{itemize}
    \item \textbf{WebAssembly (WASM)}: Ermöglicht die Ausführung von Code, der in Rust, C++ oder Go geschrieben wurde, mit höherer Ausführungsgeschwindigkeit als JavaScript. Besonders für algorithmisch komplexe Operationen (Pathfinding, physikalische Simulationen) könnte WASM in zukünftigen Versionen relevant werden.

    \item \textbf{Hybrid-Rendering}: Die Kombination von Canvas-Rendering für Performance-kritische Elemente mit \ac{DOM}-Rendering für \ac{UI}-Komponenten, wie in Atlas \ac{VTT} praktiziert, etabliert sich als Best Practice für komplexe Web-Anwendungen.
\end{itemize}

\paragraph{Ausblick}

Die Entwicklung von Atlas \ac{VTT} demonstriert, dass moderne Web-Technologien auch anspruchsvolle Echtzeit-Anwendungen in Plugin-Kontexten ermöglichen. Die identifizierten Patterns -- \ac{WebGL}-basiertes Rendering, reaktives State Management, asynchrone Ressourcenladung -- werden voraussichtlich auch in zukünftigen Projekten relevant bleiben. Die kontinuierliche Evolution von Web-Standards und Rendering-Bibliotheken wird neue Optimierungsmöglichkeiten eröffnen, während die grundlegenden architektonischen Prinzipien Bestand haben dürften.